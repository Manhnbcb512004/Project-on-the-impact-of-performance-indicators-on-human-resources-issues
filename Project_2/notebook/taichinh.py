...
# License: MIT License
#
# This Python module is part of the Credit Risk Analytics project and is
# released under the MIT License. See LICENSE.md for full licensing details.
...
# ========================================================================================
# CORE 1: NĂNG LỰC TÀI CHÍNH
# ========================================================================================

# =========================================================================================
# KPI 1.1: DTI (DEBT-TO-INCOME) - TỶ LỆ NỢ TRÊN TỔNG THU NHẬP
# =========================================================================================
def analyze_dti_foundations(df):
    # KPI Phụ 1.1.1: Phân bổ tỷ lệ nợ xấu theo các ngưỡng DTI
    ...
    # KPI Phụ 1.1.2: Phân tích sự biến thiên DTI theo Trình độ học vấn
...
# =========================================================================================
# KPI 1.2: DTI TỔNG THỂ (OVERALL DEBT-TO-INCOME) - TỶ LỆ NỢ TRÊN TỔNG THU NHẬP
# =========================================================================================
def analyze_overall_dti(df):
    ...
    # KPI Phụ 1.2.1: Mật độ phân phối DTI tổng thể giữa nhóm nợ tốt và nợ xấu
    ...
    plt.title("BIỂU ĐỒ 1.3: PHÂN PHỐI CHỈ SỐ DTI THEO TRẠNG THÁI NỢ", fontsize=14, fontweight="bold")
    plt.xlabel("Debt-to-Income Ratio (DTI)")
    plt.ylabel("Mật độ")
    plt.legend(title="Trạng thái nợ", labels=["Nợ tốt (0)", "Nợ xấu (1)", "Ngưỡng chuẩn"])
    plt.show()

    # KPI Phụ 1.2.2: Tỷ lệ nợ xấu theo từng phân khúc DTI tổng thể
    # ----------------------------------------------------------------------------------------
    ...
    plt.title("BIỂU ĐỒ 1.4: TỶ LỆ NỢ XẤU THEO TỪNG PHÂN KHÚC DTI TỔNG THỂ", fontsize=14, fontweight="bold")
...
# =========================================================================================
# KPI 1.3: NET INCOME - PHÂN TÍCH THU NHẬP RÒNG
# =========================================================================================
# KPI Phụ 3.1.1: TƯƠNG QUAN GIỮA THU NHẬP RÒNG VÀ TỶ LỆ NỢ XẤU
def analyze_income_kpi(df):
    ...
    plt.title("BIỂU ĐỒ 1.5: TƯƠNG QUAN GIỮA THU NHẬP RÒNG VÀ TỶ LỆ NỢ XẤU", fontsize=15, fontweight="bold", pad=20)
    plt.ylim(0, max(income_kpi["Bad_Loan_Rate (%)"].max(), 10) + 10)
    plt.legend()
    plt.show()

# KPI Phụ 3.1.2: Thống kê mô tả và phát hiện ngoại lệ (Outliers) thu nhập
def descriptive_analysis(df):
    ...
    plt.title("BIỂU ĐỒ 1.6: PHÂN PHỐI THU NHẬP RÒNG QUA BOXPLOT", fontsize=14)
    plt.xlabel("Trạng thái nợ (0: tốt, 1: xấu)")
    plt.ylabel("Thu nhập ròng (VNĐ)")
    plt.show()

# =========================================================================================
# KPI CHÍNH 1.4: LTI CỤ THỂ (SPECIFIC LOAN-TO-INCOME) - TỶ LỆ KHOẢN VAY TRÊN THU NHẬP
# =========================================================================================
...
def analyze_specific_lti(df):
    ...
    # KPI Phụ 1.4.1: Tỷ lệ nợ xấu theo phân đoạn LTI cụ thể
    ...
    plt.title("BIỂU ĐỒ 1.7: TỶ LỆ NỢ XẤU THEO ĐÒN BẨY KHOẢN VAY (LTI CỤ THỂ)", fontweight="bold")
    plt.ylabel("Tỷ lệ nợ xấu (%)")
...
# =========================================================================================
# KPI 1.5: THU NHẬP TRÊN ĐẦU NGƯỜI (INCOME PER DEPENDENT - IPD)
# =========================================================================================
...
def analyze_ipd(df):
    ...
    # KPI Phụ 1.5.1: Tỷ lệ nợ xấu theo mức thu nhập bình quân gia đình
    ...
    plt.title("BIỂU ĐỒ 1.8: TỶ LỆ NỢ XẤU THEO THU NHẬP BÌNH QUÂN ĐẦU NGƯỜI (IPD)", fontweight="bold")
    plt.ylabel("Tỷ lệ nợ xấu (%)")
...
# =========================================================================================
# NHÓM MỞ RỘNG CORE 1 : PHÂN TÍCH CHUYÊN SÂU & DỰ BÁO RỦI RO
# =========================================================================================
...
def plot_demographics_risk(df):
    ...
    axes[0].set_title("BIỂU ĐỒ MR1.1:TỶ LỆ NỢ XẤU THEO ĐỘ TUỔI", fontweight="bold")
    ...
    axes[1].set_title("BIỂU ĐỒ MR1.2: XU HƯỚNG RỦI RO THEO TRÌNH ĐỘ HỌC VẤN",fontweight="bold")
...
def plot_financial_depth(df):
    ...
    plt.title("BIỂU ĐỒ MR2.1: MA TRẬN RỦI RO: THU NHẬP VS LTI", fontweight="bold")
    plt.show()

    # MR2.2 Biểu đồ Pareto/concentration
    ...
    plt.title("BIỂU ĐỒ MR2.2: CƠ CẤU NỢ XẤU THEO PHÂN KHÚC LTI", fontweight="bold")
...
# --- Giải thích 
# --- Chân dung dựa trên LTI = DTI phản ánh áp lực trả nợ hàng tháng của khách hàng (tỷ lệ thanh toán nợ trên thu nhập).
# --- Phù hợp để phân loại rủi ro thanh khoản và đánh giá khả năng duy trì trả nợ ngắn hạn.
# --- Lưu ý: không thay thế hoàn toàn LTI truyền thống (loan_amount / income); nên lưu chỉ số gốc và ghi rõ nguồn.
...
# ========================================================================================
# CORE 2: UY TÍN TÍN DỤNG
# ========================================================================================

# =========================================================================================
# KPI 2.1: CREDIT SCORE ANALYSIS (PHÂN TÍCH ĐIỂM TÍN DỤNG)
# =========================================================================================
def analyze_credit_score_discriminatory_power(df):
    ...
    """
    # 2.1.1 Tính toán AUC-ROC và Gini
    ...
    # 2.2.2 KDE Plot: Overlapping Distributions
    ...
    plt.title("BIỂU ĐỒ 2.1: SỰ TRÙNG LẶP PHÂN PHỐI ĐIỂM TÍN DỤNG (OVERLAPPING DISTRIBUTIONS)", 
    ...
    plt.title('BIỂU ĐỒ 2.2: ĐƯỜNG CONG ROC - CREDIT SCORE')
    plt.legend(loc="lower right")
    plt.show()

    # 2.2.4 Phân tích Bad Rate Saturation (Bão hòa nợ xấu)
...
def analyze_advanced_risk_modules(df):
    ...
    # module 1. Bad Rate Segmentation (Bar chart 10 phân khúc)
    ...
    plt.title("BIỂU ĐỒ module 1: Sự bão hòa tỷ lệ nợ xấu (Bad Rate Saturation) trên mọi phân khúc điểm số", pad=20)
    plt.xticks(rotation=360, ha='center', fontsize=10)
    for p in ax.patches:
        ax.annotate(f"{p.get_height():.1f}%", (p.get_x() + p.get_width() / 2., p.get_height()), ha="center", va="bottom", fontweight='bold')
    plt.show()

    # module 2. Previous Defaults Analysis (Feature Importance - Random Forest)
    le = LabelEncoder()
    ...
    plt.title("BIỂU ĐỒ module 2:Các yếu tố hành vi thực tế đang chi phối rủi ro mạnh hơn Điểm tín dụng", pad=20)
    plt.show()

    # module 3. Interaction (Income Bins x LTI Bins Heatmap)
    ...
    plt.title("BIỂU ĐỒ module 3: Ma trận tương tác cho thấy rủi ro cực đoan tại nhóm Thu nhập thấp & LTI Cao", pad=20)
    plt.show()

    # module 4. Behavioral Signals (Delinquency x Previous Defaults Heatmap)
    ...
    plt.title("BIỂU ĐỒ module 4: Tín hiệu hành vi: Lịch sử vỡ nợ là biến số dự báo nợ xấu mạnh nhất", pad=20)
    plt.show()

    # module 5. Risk Modeling Metrics (ROC Curve) - Đã đồng nhất phong cách
...
# ========================================================================================
# KPI 2.2 : Delinquency Freq và Previous Defaults
# ========================================================================================

# =========================================================================================
# KPI 2.2.1: BEHAVIORAL TRANSITION MATRIX (MA TRẬN DỊCH CHUYỂN HÀNH VI)
# =========================================================================================
def analyze_behavioral_transition_matrix(df):
    ...
    plt.title("BIỂU ĐỒ 2.3: MA TRẬN DỊCH CHUYỂN NHÓM RỦI RO (BEHAVIORAL TRANSITION)", fontsize=14, fontweight='bold')
    ...
    print("\n" + "="*80)
    print("KPI 2.2.1: INSIGHT DỊCH CHUYỂN HÀNH VI")
    print("="*80)
...
# =========================================================================================
# KPI 2.2.2: DELINQUENCY ESCALATION CURVE (ĐƯỜNG CONG LEO THANG RỦI RO)
# =========================================================================================
def analyze_delinquency_escalation_curve(df):
    ...
    plt.title("BIỂU ĐỒ 2.4: ĐƯỜNG CONG LEO THANG RỦI RO (DELINQUENCY ESCALATION)", fontsize=14, fontweight='bold')
    ...
    print("\n" + "="*80)
    print("KPI 2.2.2: INSIGHT LEO THANG RỦI RO HÀNH VI")
    print("="*80)
...
# =========================================================================================
# KPI 2.2.3: BEHAVIORAL PERSISTENCE & EARLY WARNING ANALYSIS
# =========================================================================================
def analyze_behavioral_persistence_early_warning(df):
    ...
    if 'Delinquency_Freq' in df.columns:
        ...
        logging.warning("KPI 2.2.3: Both 'Delinquency_Freq' and 'Payment_History' are missing. Defaulting to 0.")
    ...
    plt.title("BIỂU ĐỒ 2.5: MA TRẬN CẢNH BÁO SỚM (BEHAVIORAL PERSISTENCE vs LEVERAGE)", fontsize=14, fontweight='bold')
    ...
    plt.title("BIỂU ĐỒ 2.6: TƯƠNG QUAN MỤC ĐÍCH VAY, ĐỘ LÌ HÀNH VI VÀ NỢ XẤU", fontsize=14, fontweight='bold')
    ...
    print("\n" + "="*80)
    print("KPI 2.2.3: INSIGHT VỀ ĐỘ LÌ HÀNH VI & CẢNH BÁO SỚM")
    print("="*80)
    ...
    return ew_matrix
# =========================================================================================
# KPI 2.2.4: BEHAVIORAL CLUSTERING (PHÂN NHÓM CHÂN DUNG HÀNH VI ẨN)
# =========================================================================================
def analyze_behavioral_clustering(df):
    ...
    plt.title("BIỂU ĐỒ 2.7: PHÂN CỤM PERSONA QUA PCA", fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.2)
    plt.show()
...
# =========================================================================================
# KPI 2.2.5: PORTFOLIO CONCENTRATION RISK (RỦI RO TẬP TRUNG DƯ NỢ)
# =========================================================================================
def analyze_portfolio_concentration_risk(df):
    ...
    """
    print("\n" + "="*80)
    print("KPI 2.2.5: PHÂN TÍCH RỦI RO TẬP TRUNG (PORTFOLIO CONCENTRATION)")
    print("="*80)
    ...
    fig = px.treemap(conc_df, 
                     ...
                     title='BIỂU ĐỒ 2.8: TREEMAP TẬP TRUNG DƯ NỢ & TỶ LỆ NỢ XẤU')
    fig.show()
    ...
    plt.title("BIỂU ĐỒ 2.9: TƯƠNG QUAN DƯ NỢ, QUY MÔ KH VÀ RỦI RO", fontsize=14, fontweight='bold')
...
# =========================================================================================
# KPI 2.2.6: EXPLAINABLE AI FOR BEHAVIORAL RISK (XAI)
# =========================================================================================
def analyze_explainable_behavioral_risk(df):
    ...
    """
    print("\n" + "="*80)
    print("KPI 2.2.6: GIẢI THÍCH MÔ HÌNH RỦI RO (EXPLAINABLE AI - SHAP)")
    print("="*80)
    ...
    plt.figure(figsize=(10, 6))
    plt.title("BIỂU ĐỒ 2.10: TÁC ĐỘNG CỦA CÁC BIẾN ĐẾN XÁC SUẤT NỢ XẤU (SHAP SUMMARY)")
    ...
    plt.title("BIỂU ĐỒ 2.11: TRỌNG SỐ TÁC ĐỘNG CỦA CÁC BIẾN HÀNH VI (%)", fontsize=12, fontweight='bold')
